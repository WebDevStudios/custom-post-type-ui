<?php
/**
 * Generate documentation for hooks in CPTUI
 *
 * Borrowed originally from WooCommerce.
 */
class CPTUI_HookFinder {
	...
	private static function get_hook_link( $hook, $details = array() ) {
		if ( ! empty( $details['class'] ) ) {
			...
			// $link = 'https://github.com/WebDevStudios/custom-post-type-ui/search?utf8=%E2%9C%93&q=' . $hook;
		}
	...
	public static function process_hooks() {
		...
		$html   = file_get_contents( 'cptui-docs/tree.html' );
		...
		file_put_contents( 'cptui-docs/hook-docs.html', $header . ob_get_clean() . end( $footer ) );
		echo "Hook docs generated :)\n";
	}
}

CPTUI_HookFinder::process_hooks();
